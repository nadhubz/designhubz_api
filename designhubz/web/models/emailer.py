...
class EmailerManager(models.Manager):
    ...
    def change_email_context(self, user):
        d = self.get_common_context()
        d['link_href'] = user.get_change_email_link()
        d['link_text'] = _('Confirm new email')
        return d

    def resend_reg_confirmation_context(self, user):
        d = self.get_common_context()
        d['link_href'] = user.get_confirmation_link()
        d['link_text'] = _('Confirm registration')
        return d

    def reset_password_context(self, user):
        d = self.get_common_context()
        d['link_href'] = user.get_reset_link()
        d['link_text'] = _('Reset password')
        return d

    def signup_context(self, user):
        d = self.get_common_context()
        d['link_href'] = user.get_confirmation_link()
        d['link_text'] = _('Confirm registration')
        return d
    ...
    def mail_user(self, user, code):
        ...
        if addtitional_context is not None:
            d.update(addtitional_context(user))
